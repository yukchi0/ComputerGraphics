...
가장 까다로웠던 부분은 소나 파동이 벽에 닿았을 때 반사파가 자연스럽게 생성되도록 만드는 것이었다.

초기에는 파동 반경이 벽에 도달하는 순간을 단순히 거리 비교로 판단했는데, 매 프레임마다 반사 이벤트가 중복 발생하거나 반사파가 벽 내부에서 생성되는 문제가 있었다.

이를 해결하기 위해 `Raycaster`를 16방향으로 쏴서 파동 원점으로부터 현재 반경(`radius`)과 이전 반경(`prevRadius`) 사이에 벽과 교차한 지점만 반사 원점으로 채택하는 방식으로 바꿨다.
...
function getCollisionPoints(cx, cz, radius, prevRadius) {
  ...
  for (let i = 0; i < DIRS; i++) {
    ...
    if (wallHits.length > 0) {
      ...
      // prevRadius ~ radius 사이에서 처음 교차한 지점만 채택
...
또한 같은 벽 충돌 지점에서 반사파가 여러 번 생성되지 않도록 `_collided` Set에 충돌 좌표를 기록해 중복을 걸렀다. 반사파는 원본 파동의 속도·거리를 각각 0.8배·0.4배로 줄여 에너지 감쇠를 표현하였다.
...
hits.forEach(pt => {
  ...
  activePulses.push(new Pulse(
    ...
    this.maxDist * 0.4,  // 최대 거리 40%
    this.speed * 0.8,    // 속도 80%
    false, true
  ));
});
```

이 방식으로 파동이 벽 모서리에 부딪혔을 때 자연스럽게 여러 개의 반사파가 갈라져 나오면서도 동일 지점에서 중복 생성되는 문제를 해결할 수 있었다.
